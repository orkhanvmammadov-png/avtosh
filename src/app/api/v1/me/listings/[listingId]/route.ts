...
export const GET = createApiHandler(async ({ request, requestId, params }) => {
  ...
  const paymentRequired = await paymentRequiredFor(listing.id, auth.user.id, listing.status);
  return apiSuccess(
    { listing, moderation_feedback: moderationFeedback, payment_required: paymentRequired },
    { requestId },
  );
});
