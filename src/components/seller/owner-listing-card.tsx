...
export function OwnerListingCard({ listing }: { listing: OwnerCardDto }) {
  ...
  return (
    <article
      className="flex gap-3 rounded-card border border-line bg-raised p-3 shadow-card transition-shadow hover:shadow-raised"
    ...
    >
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-1">
        <div className="flex flex-wrap items-center gap-2">
          <span
            className={`rounded-md border px-2 py-0.5 text-xs font-semibold ${TONE_CLASSES[presentation.tone]}`}
        ...
        {listing.premiumUntil !== null || listing.boostUntil !== null ? (
          <p className="mt-1 text-xs font-medium text-navy" data-testid="owner-promotions">
            {listing.premiumUntil !== null ? (
              <span className="mr-3 text-premium-deep" data-testid="owner-premium-until">
                {SELLER.premiumActive} — {formatDateAz(listing.premiumUntil)} {SELLER.promotionUntil}
              </span>
            ) : null}
            {listing.boostUntil !== null ? (
              <span className="text-boost-deep" data-testid="owner-boost-until">
      ...
      {href !== null && presentation.action.kind !== "none" ? (
        <div className="flex shrink-0 flex-col items-stretch justify-center gap-2">
          <Link
            href={href}
            className="inline-flex min-h-12 items-center justify-center rounded-control border border-line px-3 text-sm font-semibold text-primary transition-colors hover:border-line-strong hover:bg-surface"
          ...
          {listing.status === "ACTIVE" ? (
            <Link
              ...
              className="inline-flex min-h-12 items-center justify-center rounded-control bg-primary px-3 text-sm font-semibold text-white transition-colors hover:bg-primary-hover"
